...
if __name__ == "__main__":
    ...
    property_dict = {}
    onnx_name = "./nets/cifar_base_kw.onnx"
    nn_name = "cifar_base_kw"
    property_dict[onnx_name] = list()
    vnnlib_path = "./vnnlib_base/"
    with open("./cifar10ovalbase_01.txt", "r") as f:
        lines = f.readlines()
    with open("./OVALidToImageIndex.json", "r") as p:
        data = json.loads(p.read())
    
    for line in lines:
        line_data = eval(line.strip())
        line_data = [int(line_data[0]), float(line_data[1])]
        line_index = line_data[0]
        eps = line_data[1]
        imag_idx = data[str(line_index)]
        
        filename = vnnlib_path + f"{nn_name}-img{imag_idx}-eps{eps}.vnnlib"
        comment = f"Adversarial robustness property for network {nn_name}. l_inf radius: {eps}, " \
                    f"CIFAR10 test image n. {imag_idx}."
        _, ground_truth, _, domain = load_cifar_oval_kw_1_vs_all(
            nn_name, int(imag_idx), epsilon=eps,
            no_verif_layers=True)
        write_adversarial_robustness_vnnlib(filename, comment, domain, ground_truth)
        property_dict[onnx_name].append(filename)

    timeout = 1000
    create_benchmark_csv("oval21_base_instances.csv", property_dict, timeout)
