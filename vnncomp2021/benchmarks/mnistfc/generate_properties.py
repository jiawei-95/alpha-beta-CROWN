...
def load_data(data_dir: str = "./tmp", num_imgs: int = 25, random: bool = False) -> tuple:
    ...
    if random:
        ...
        loader_test = DataLoader(mnist_test, batch_size=100, shuffle=False)
...
def create_instances_csv(net, num_props: int = 15, path: str = f"mnistfc_instances.csv"):
...
if __name__ == '__main__':
    ...
    images, labels = load_data(num_imgs=num_images, random=False)
    
    with open("mnistL4_01.txt", "r") as f:
        lines = f.readlines()
    vnnlib = open("mnistL4_instances.csv", "w")
    for line in lines:
        ...
        spec_path = f"./MNISTL4/prop_{i}_{eps}.vnnlib"

        save_vnnlib(input_bounds, label, spec_path)
        vnnlib.write(f"mnist-net_256x4.onnx, ./MNISTL4/prop_{i}_{eps}.vnnlib,1000\n")
    # create_instances_csv("mnist-net_256x2.onnx")
    vnnlib.close()
